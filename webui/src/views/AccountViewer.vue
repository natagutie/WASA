  ...
  methods: {
    async IfFollow() {
      try {
        ...
        console.log("ifFollwç" + this.ifFollowing);
        if (this.ifFollowing == false) {
          document.getElementById("follow").innerHTML = "follow";
        } else if (this.ifFollowing == true) {
    ...
    async getUserComments(photoID) {
      try {
        ...
        this.errormsg = "Failed to get comments";
      }
    },
    ...
    async IfBanned() {
      try {
        ...
        let ifban = await this.$axios.get(
          "/username/" + this.currentUsername + "/ban/" + this.profileUsername,
          {
            headers: {
              Authorization: "Bearer " + localStorage.getItem("userID")
            }
      ...
      try {
        ...
        if (this.ifFollowing) {
          await this.$axios.delete(
            "/username/" + this.currentUser + "/follow/" + this.userUsername,
            {
              headers: {
                Authorization: "Bearer " + localStorage.getItem("userID")
              }
          ...
          await this.$axios.delete(
            "/username/" + this.userUsername + "/follow/" + this.currentUser,
            {
              headers: {
                Authorization: "Bearer " + localStorage.getItem("userID")
              }
...
<template>
  ...
  <div v-if="this.pictureCount > 0">
    <div class="card-columns">
      <div v-for="photo in this.photoList" :key="photo.photoID">
